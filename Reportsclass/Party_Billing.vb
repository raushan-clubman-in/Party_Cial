...
Public Class Party_Billing
    ...
    Public Function ReportDetails(ByVal Billno As String, ByVal Billtype As String)
        Try
            Dim Arrtaxamount, Arramount, Arrtotalamount As Double
            Dim Restaxamount, Resamount, Restotalamount As Double
            Dim Halltaxamount, Hallamount, Halltotalamount, ADVANCE, Totalamount As Double
            Call Randomize()
            ...
            ssql = ssql & " HALLCODE,HALLAMOUNT,HALLTAXAMOUNT,"
            ...
            If dt.Rows.Count > 0 Then
                ...
                Filewrite.WriteLine(Space(11) & "MNAME :" & Mid(dt.Rows(0).Item("MNAME"), 1, 30) & Space(30 - Len(Mid(dt.Rows(0).Item("MNAME"), 1, 30))) & Space(2) & "|")
                Filewrite.WriteLine("|" & Space(79) & "|")
                Filewrite.Write("|" & Space(2) & Trim(Billtype) & " NO    :" & Mid(Billno, 1, 7) & Space(7 - Len(Mid(Billno, 1, 7))))
                Filewrite.WriteLine(Space(11) & Trim(Billtype) & " DATE :" & Mid(Format(dt.Rows(0).Item("bookingdate"), "dd/MM/yyyy"), 1, 10) & Space(10 - Len(Mid(Format(dt.Rows(0).Item("bookingdate"), "dd/MM/yyyy"), 1, 10))) & Space(20) & "|")
                Filewrite.WriteLine("|" & Space(79) & "|")
                ...
                If DT1.Rows.Count > 0 Then
                    ...
                    Next
                End If
                ssql = "|" & Space(48) & " TOTAL RECEIPT    :" & Space(11 - Len(Mid(Format(ADVANCE_ANOUNT(Billno), "0.00"), 1, 11))) & Mid(Format(ADVANCE_ANOUNT(Billno), "0.00"), 1, 11) & Space(1) & "|"
                Filewrite.WriteLine(ssql)
                Filewrite.WriteLine("|" & Space(79) & "|")
                ...
                'ARRANGEMENT DETAILS
                ssql = "  SELECT BOOKINGTYPE,BOOKINGNO,ITEMCODE,ITEMDESCRIPTION,UOM,QTY,RATE,TAXPERC,TAXAMOUNT,ROUNDOFF,AMOUNT,TOTALAMOUNT "
                ...
                ssql = ssql & "|" & Space(11 - Len(Mid(Format(Arrtotalamount, "0.00"), 1, 11))) & Mid(Format(Arrtotalamount, "0.00"), 1, 11) & "|"
                Filewrite.WriteLine(ssql)

                'CHARGABLE ITEM DETAILS
                ssql = " SELECT BOOKINGTYPE,BOOKINGNO,ITEMCODE,ITEMDESCRIPTION,UOM,QTY,RATE,TAXPERC,TAXAMOUNT,ROUNDOFF,AMOUNT,TOTALAMOUNT  FROM VIEW_PARTY_RESTAURANT"
                ssql = ssql & " WHERE  BOOKINGTYPE='" & Billtype & "' AND BOOKINGNO=" & Billno
                dt = gconnection.GetValues(ssql)
                If dt.Rows.Count > 0 Then
                ...
                ssql = ssql & "|" & Space(11 - Len(Mid(Format(Restotalamount, "0.00"), 1, 11))) & Mid(Format(Restotalamount, "0.00"), 1, 11) & "|"
                Filewrite.WriteLine(ssql)
                Filewrite.WriteLine("|" & StrDup(79, "-") & "|")
                Totalamount = Halltotalamount + Restotalamount + Arrtotalamount
                ssql = "|" & Space(56) & "TOTAL AMOUNT :"
                ...
                ssql = "|" & Space(56) & "BALANCE      :"
                ssql = ssql & Space(9 - Len(Mid(Format((Totalamount - ADVANCE_ANOUNT(Billno)), "0.00"), 1, 9))) & Mid(Format((Totalamount - ADVANCE_ANOUNT(Billno)), "0.00"), 1, 9) & "|"
                Filewrite.WriteLine(ssql)
                Filewrite.WriteLine("|" & Space(79) & "|")
                Filewrite.WriteLine("|" & Space(2) & "Rupess :" & Mid(ConvertRupees((Totalamount - ADVANCE_ANOUNT(Billno))), 1, 65) & Space(65 - Len(Mid(ConvertRupees((Totalamount - ADVANCE)), 1, 65))) & Space(4) & "|")
                Filewrite.WriteLine("|" & Space(79) & "|")
                Filewrite.WriteLine("|" & Space(79) & "|")
                Filewrite.WriteLine("|" & Space(79) & "|")
    ...
    Private Sub Arrangement_Heading()
        ...
        Filewrite.WriteLine("|" & StrDup(79, "-") & "|")
        Filewrite.WriteLine("|SNO |         FACILITY    | UOM | QTY |   RATE |TAXAMOUNT| AMOUNT  |TOTALAMOUNT|")
        Filewrite.WriteLine("|" & StrDup(79, "-") & "|")
        pagesize = pagesize + 4
    End Sub
